...
function removeSymbol(){
  if(symbol.length > 0){    
    ...
    var symbolX = symbol[symbol.length-1].getAttribute("x");
    var symbolY = symbol[symbol.length-1].getAttribute("y");  
    checkForLoser(playerTurn, (Math.floor(symbolX/boxSize)), (Math.floor(symbolY/boxSize)));  
    document.getElementById("box" + Math.floor(symbolX/boxSize) + "_" + Math.floor(symbolY/boxSize)).classList = "neutral";
...
function checkForLoser(subject, x, y){
  ...
  for(var i=line.length-2, length=line.length-8; i>=length; i-=2){
    if(i >= 0){  
...
function displayNextSlide(){
  if(slide == 1){
    ...
    if(gameMode == "Tetris-Style"){
      document.getElementById("tutorialText").innerHTML += "<h2><u>Note:</u></h2><br><p>&emsp; Tetris-Style is a self-made and does not exist normally. This is an experiment to see how gameplay will be affected if winning streaks dispapear.</p><button class='nextButton' onclick='displayNextSlide()'>Next: <i class='arrow right'></i></button>";  
      slide = 3;   
    }  
    else{
